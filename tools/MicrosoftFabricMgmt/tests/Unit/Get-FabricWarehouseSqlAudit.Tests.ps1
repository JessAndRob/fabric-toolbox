...
<# Behavior tests for Get-FabricWarehouseSqlAudit: GET .../warehouses/{id}/settings/sqlAudit. #>
...
Describe 'Get-FabricWarehouseSqlAudit' -Tag 'UnitTests' {
    It 'GETs the sqlAudit settings endpoint' {
        $null = Get-FabricWarehouseSqlAudit -WorkspaceId 'ws-1' -WarehouseId 'wh-1'
    ...
    It 'enriches with WorkspaceName and the type name' {
        $r = Get-FabricWarehouseSqlAudit -WorkspaceId 'ws-1' -WarehouseId 'wh-1'
